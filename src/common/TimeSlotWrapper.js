...
function getDefaultTime(date) {
  ...
  if(typeof(publicHoliday) !== 'undefined') {
    return {
      color: '#858585',
      text: `Fermeture - ${publicHoliday.nom_jour_ferie}`,
    }
  }
    ...
    if(typeof(time) === 'undefined') {
      ...
      return (
        <div className='rbc-time-slot'>
          <div className='rbc-event' style={style} title="Sur rendez-vous">
            {(this.props.value.getHours()<10 && this.props.value.getMinutes() < 30) && 'Sur rendez-vous'}
          </div>
        </div>
      )
    }
    ...
    return (
      <div className='rbc-time-slot'>
        <div className='rbc-event' style={style}>
          {(this.props.value.getHours() === 9 && this.props.value.getMinutes() === 0 || (this.props.value.getHours() === 12 && this.props.value.getMinutes() === 30)) && time.text}
        </div>
      </div>
    )
  }
